...
CUDA_VISIBLE_DEVICES=${GPU_IDS} python -m torch.distributed.launch --nproc_per_node=${NUM_GPUS} helpers/segmentor_trainer.py \
 --name segmenter_synthetic_cityscapes_128 \
 --dataset "cityscapes" --max_dim 1024 --dim 128 \
 --save_latest_freq 1 --num_workers 16 --gpu_ids ${GPU_IDS} \
 --batch_size 16 \
 --eval_freq 10  --log_freq 50 \
 --x_model deeplabv3 --niter 300 --x_pretrained_path "./checkpoints/resnet101-imagenet.pth" \
 --x_not_restore_last \
 --x_synthetic_dataset \
 ...
 --soft_sem_seg \
 --estimated_cond --estimator_load_path "checkpoints/cond_estimator_512" \
