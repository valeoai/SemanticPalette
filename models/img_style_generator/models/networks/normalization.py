...
class ACE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, ACE_Name=None, status='train', spade_params=None, use_rgb=True, dir_path=None):
        super().__init__()
    ...
    def forward(self, x, segmap, style_codes=None, obj_dic=None):
        ...
        if self.use_rgb:
            ...
            if self.status == 'UI_mode' or self.status == 'use_style':
                ############## hard coding

                for i in range(b_size):
                    if i > 1 and self.status == 'UI_mode':
                        break
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            if obj_dic is None:
                                ...
                                middle_avg[i].masked_scatter_(segmap.bool()[i, j], component_mu.float()) # component_mu)
                    ...
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            ...
                            if self.status == 'test' and self.save_npy and self.ACE_Name=='up_2_ACE_0':
                                ...
                                folder_path = os.path.join(self.dir_path, 'style_codes', im_name, str(j))
                            ...
                            if self.status == 'save_style' and self.ACE_Name=='up_2_ACE_0':
                                obj_dic[str(j)] = {}
                                obj_dic[str(j)]['ACE'] = style_codes[i][j]
