...
from models.img_style_generator.models.networks.normalization import SPADE, ACE
...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt, Block_Name=None, use_rgb=True):
        ...
        self.ace_0 = ACE(our_norm_type, fin, 3, ACE_Name= Block_Name + '_ACE_0', status=self.status, spade_params=[spade_config_str, fin, opt.num_semantics], use_rgb=use_rgb, dir_path=opt.log_path)
        ###########  Modifications 1


        ###########  Modifications 1
        self.ace_1 = ACE(our_norm_type, fmiddle, 3, ACE_Name= Block_Name + '_ACE_1', status=self.status, spade_params=[spade_config_str, fmiddle, opt.num_semantics], use_rgb=use_rgb, dir_path=opt.log_path)
        ###########  Modifications 1

        if self.learned_shortcut:
            self.ace_s = ACE(our_norm_type, fin, 3, ACE_Name= Block_Name + '_ACE_s', status=self.status, spade_params=[spade_config_str, fin, opt.num_semantics], use_rgb=use_rgb, dir_path=opt.log_path)

    def set_status(self, status):
        self.ace_0.status = status
        self.ace_1.status = status
        if self.learned_shortcut:
            self.ace_s.status = status
