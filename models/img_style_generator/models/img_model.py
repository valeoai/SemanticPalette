...
import models.img_style_generator.models.networks as networks
...
class ImgModel(torch.nn.Module):
    ...
    def forward(self, data, fake_data={}, tgt_data=None, mode="", log=False, global_iteration=0, suffix="", has_gt=True, dis="both"):
        ...
        obj_dic = data["obj_dic"] if "obj_dic" in data else None
        ...
        if mode == 'generator':
            ...
            with torch.no_grad():
                fake_img, _ = self.generate_fake(real_seg, real_image, obj_dic, log=log, global_iteration=global_iteration)
            return {"img": fake_img}
    ...
    def generate_fake(self, real_seg, real_image, obj_dic, compute_kld_loss=False, log=False, global_iteration=0):
        # z = None
        kld_loss = None
        # if self.opt.use_vae:
        ...
        fake_image = self.netG(real_seg, real_image, obj_dic=obj_dic)
