...
class GeneratorStatisticsTester:
        ...
        with Engine(self.opt) as engine:
            ...
            for i in range(self.opt.niter):
                ...
                if is_main:
                    ...
                    print(f"for building: {np.mean(100 * tgt_cond, axis=0)[11]} (real) {np.mean(100 * cond, axis=0)[11]} (fake)")
                    print(f"for car: {np.mean(100 * tgt_cond, axis=0)[26]} (real) {np.mean(100 * cond, axis=0)[26]} (fake)")
...
if __name__ == "__main__":
    ...
    GeneratorStatisticsTester(opt).run()
